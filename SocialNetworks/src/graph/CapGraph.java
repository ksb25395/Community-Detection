...
public class CapGraph implements Graph {
	...
	/*
	 *Calculate the edge-betweenness for all the edges of the CapGraph and 
	 * store it in a hashMap -- edgeBetweennessMap
	 */
		...
		for(Node graphNode : graphNodes.values()) {
			...
			while(!q.isEmpty()) {
				...
				for(Edge w : v.getAdjList()) {
					...
					//its level  to one higher than its predecessor v
					...
					if(depth.get(otherVertex) == depth.get(v.getLabel()) + 1) {
						...
						/*
						 *	we add the node v to the list of predecessors 
							of the vertex otherVertex. 
			...
			/*
			 * delta is a hashmap with key as the vertex label and 
			 * value as the sum of centrality updates from edges originating 
			 * from it.
			 */
			...
			while(!vertexStack.isEmpty()) {
				...
				for(Node vertex : pMap.get(w.getLabel())) {
					 ...
					 * vertex plus one.
					 */
					double value = shortestPathCount.get(vertex.getLabel())*(delta.get(w.getLabel())/shortestPathCount.get(w.getLabel()) + 1);
	...
	public CapGraph reverse() {
		CapGraph R = new CapGraph();
	 ...
	 * from the graph and edge betweenness map 
	 * and the adjacent lists of the head and
	 * the tail
	 */
	...
	/*
	 * Returns the list of edges with the maximum
	 * measure of betweenness centrality. We then
	 * remove all the edges with the maximum BW
	 * centrality measure
	 */
	...
	/*
	 * Program driver for the Girvan Newman algorithm for
	 * detecting communities using edge betweenness
	 * centrality measure
	 */
